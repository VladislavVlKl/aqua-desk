...
| Файл | Назначение |
|---|---|
| `app.js` | Весь UI и бизнес-логика (~8000 строк) |
| `db.js` | Обёртки над Supabase (`DB.*`), ~1900 строк |
...
| `notifications.js` / `notifications-ui.js` | Уведомления |
| `style.css`, `analytics.css` | Стили |
...
### CEO (renderCeoApp)
Дашборд (выручка/ПТ/дежурства/группы по филиалам), ЗП-сводка, группы (просмотр), операционка, планы.
