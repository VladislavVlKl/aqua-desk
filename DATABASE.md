...
(миграция 20260704150000). Фронт откачен в `JWT_MODE='diagnostic'` (2026-07-06):
работает под anon, JWT только запрашивается и логируется. Политики authenticated
остаются в БД (аддитивны). Этап 3 (закрытие anon по таблицам) — не начат.
...
**notif_dedup** — атомарная защита от повторной отправки уведомлений: `rule_key PK, created_at` (миграция 20260910120000). Отправитель (Edge Function `daily-reminder` / `remind.js`) делает INSERT `rule_key`; успех = зарезервировал и шлёт, unique_violation = уже слали в окне → пропуск. Заменяет ненадёжный select-потом-insert по `notifications_queue` (двоил при сбое чтения).

**pt_sequence_survey** — опросник сверки порядковых списаний ПТ (тест, `SEQ_SURVEY` в config.js): `round, trainer_id, client_id, branch, system_next, system_total, matches, final_next, final_total, comment, is_manual, answered_at, debt_shown, debt_final`
> `debt_shown`/`debt_final` (миграция 20260909120000) — раунд 2: показанное и подтверждённое тренером число долговых занятий (правится в той же карточке, рядом с пакетом).
...
> ⚠️ Только РУЧНЫЕ премии/штрафы. Перерасчёт ПТ (сверка с 1С) сюда НЕ пишется (был баг: голый INSERT ломался об unique-ключ при повторе) — он живёт отдельной статьёй «Разница от пересчёта», см. ниже.

**pt_mismatch_flags** — расхождение остатка ПТ с 1С (беклог #26): `client_id, trainer_id, branch, status (open|resolved|rejected|returned), system_balance_at_flag, trainer_note, trainer_suggested, corrected_balance, fot_delta, balance_before_resolve, coordinator_note, returned_by, returned_at, resolved_by, resolved_at`. Частичный unique — один активный (`open` или `returned`) на клиента.
> `returned` — координатор вернул заявку тренеру на уточнение (`coordinator_note` + пуш); тренер повторяет заявку → `open` (та же строка, `created_at` сбрасывается). ФОТ-разница при перерасчёте = (ТЕКУЩИЙ `clients.balance` − введённый остаток по 1С) × ставка, «сегодня vs сегодня» (не снимок `system_balance_at_flag`); фактический остаток на момент перерасчёта пишется в `balance_before_resolve`.
> Тренер жмёт «⚠ Не совпадает с 1С» → `open`. Координатор/старший сверяет и «Пересчитать» (`resolvePtMismatch`): ставит `clients.balance`, закрывает флаг, пишет `fot_delta` (>0 доначислить, <0 снять) и `audit_log action='pt_recalc'`. **`fot_delta` — единственный источник статьи ЗП «Разница от пересчёта»** (`DB.getRecalcByTrainer`/`getRecalcAdjustments` в db.ops.js): резолвнутые флаги за месяц `resolved_at`, знаковая сумма, разбивка по клиентам. Входит в `calcSalary` параметром `recalcSum` (отдельно от bonus/penalty), в total. Индекс `idx_ptmf_trainer_resolved (trainer_id, status, resolved_at)`. Supabase-only (API-эндпоинта нет). Миграция 20260913120000.
