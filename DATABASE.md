...
**pt_mismatch_flags** — расхождение остатка ПТ с 1С (фича, `PT_MISMATCH` в config.js): `client_id, trainer_id, branch, system_balance_at_flag, trainer_note, trainer_suggested, status(open|resolved|rejected), corrected_balance, fot_delta, resolved_by, resolved_at, reject_reason, created_at`
> Тренер отмечает на карточке клиента (кнопка «⚠ Не совпадает с 1С») → пуш старшим+координаторам филиала (rule_key `pt_mismatch`) → координатор («Контроль») или старший («Ещё») сверяет с 1С и делает **перерасчёт** (`resolvePtMismatch`): ставит `clients.balance = остаток по 1С` + (опц.) корректировка ФОТ строкой `month_adjustments` (`fot_delta = −(after−before)×RATES.pt[cat]`: остаток ↑ → penalty, ↓ → bonus). Всё в `audit_log` (`action='pt_recalc'`). `UNIQUE(client_id) WHERE status='open'` — один открытый флаг на клиента. RLS anon+authenticated. Миграция 20260911120000. Данные: `db.ops.js` (`flagPtMismatch`/`getMismatchFlag`/`getPtMismatchFlags`/`resolvePtMismatch`/`rejectPtMismatch`), UI: `app.client.js` (кнопка+модал), `app.admin-ops.control.js` (`_mmCard`/`renderMismatchFlags`/`doResolveMismatch`/`doRejectMismatch`), `app.senior.js` (кнопка в «Ещё»). Анонс — `APP_UPDATE`.
